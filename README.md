...
6. `distance` 计算2个时间的距离 接收2个参数，返回一个miment对象
    ...
    > 只传一个起始时间的时候，返回起始时间 - miment当前时间

    > 起始时间和结束时间都有传的时候，返回 起始时间 - 结束时间

        moment().distance('2018-04-10 00:00:00')   // 返回 '2018-04-10 00:00:00' - moment()
        moment().distance('2018-04-10 00:00:00', new Date())   // 返回 '2018-04-10 00:00:00' - new Date()


6. `firstDayOfWeek` **获取 本周的第一天(周日)**  不需要参数
